...
const ReviewerMakerMain = () => {
  ...
  // LocalStorage keys
  const STORAGE_KEYS = {
    API_KEY: 'reviewerMaker_apiKey',
    EXTRACTIONS: 'reviewerMaker_extractions',
    LAST_TEXT: 'reviewerMaker_lastText',
    SELECTED_MODE: 'reviewerMaker_selectedMode'
  };
  ...
  useEffect(() => {
    try {
      ...
      const savedExtractionsData = localStorage.getItem(STORAGE_KEYS.EXTRACTIONS);
      if (savedExtractionsData) {
        const parsedExtractions = JSON.parse(savedExtractionsData);
        setSavedExtractions(parsedExtractions);
      }

      // Load last input text
      const savedText = localStorage.getItem(STORAGE_KEYS.LAST_TEXT);
      if (savedText) {
      ...
      if (savedMode && ['normal', 'sentence', 'keywords'].includes(savedMode)) {
  ...
  // Save extractions to localStorage whenever they change
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEYS.EXTRACTIONS, JSON.stringify(savedExtractions));
    } catch (error) {
      console.error('Error saving extractions to localStorage:', error);
    }
  }, [savedExtractions]);

  // Save input text to localStorage whenever it changes (with debounce)
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      try {
        localStorage.setItem(STORAGE_KEYS.LAST_TEXT, inputText);
      } catch (error) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEYS.SELECTED_MODE, selectedMode);
    } catch (error) {
  ...
  const clearAllStoredData = () => {
    try {
      ...
      alert('All stored data cleared successfully!');
    } catch (error) {
...
${inputText}`;
  ...
  const HomeView = () => (
    <div className="min-h-screen flex items-center justify-center py-12">
      <div className="max-w-6xl h- mx-auto px-4">
        ...
        {/* Storage Status */}
        <div className="pixel-window bg-gray-100 p-6 mb-10 mt-10 h-15">
          <div className="pixel-title-bar bg-green-600 text-white p-3 mb-4">
            <span className="pixel-font-heading text-sm">STORAGE STATUS</span>
          </div>
          <div className="flex flex-wrap gap-4 items-center">
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 pixel-icon ${apiKey ? 'bg-green-500' : 'bg-red-500'}`}></div>
              <span className="pixel-font-body text-sm">API Key: {apiKey ? 'Saved' : 'Not Set'}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 pixel-icon bg-blue-500"></div>
              <span className="pixel-font-body text-sm">Saved Extractions: {savedExtractions.length}</span>
            </div>
            <button
              onClick={clearAllStoredData}
              className="pixel-button-sm bg-red-500 hover:bg-red-600 text-white px-3 py-2 ml-auto"
            >
              <span className="pixel-font-heading text-xs">CLEAR ALL DATA</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-center mb-12">
          ...
          >
            <FileText className="w-10 h-11 mb-13 mt-10" />
  ...
  const ExtractView = () => (
    <div className="min-h-screen grid place-items-center mt-50">
 ...
 <div>           <h2 className="pixel-font-heading  text-3xl flex items-center justify-center  ">SAVED EXTRACTIONS</h2></div>
      <div className="pixel-window bg-gray-100 p-8">
        ...
        {savedExtractions.length === 0 ? (
          ...
          export default ReviewerMakerMain;